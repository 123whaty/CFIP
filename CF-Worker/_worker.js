...
// ===== 可按需修改的部署参数 =====
// 我的VL密钥：VLESS UUID，客户端节点和 Worker 校验必须保持一致。
// 反代IP：直连目标失败时使用的 fallback 代理地址，可写成 host 或 host:port。
let 我的VL密钥 = '04c808e2-0b59-47b0-a54b-32fc7ef1c902';//UUID
let 反代IP = 'proxyip.cmliussss.net'; //反代IP

export default {
  async fetch(访问请求) {
    // WebSocket 请求是 VLESS 实际传输通道；普通 HTTP 请求用于展示 /sub 节点信息。
    if (访问请求.headers.get('Upgrade') === 'websocket') {
      // 从访问路径里读取 ip=xxx 参数，允许同一个 Worker 动态指定反代 IP。
      const 读取路径 = decodeURIComponent(访问请求.url.replace(/^https?:\/\/[^/]+/, ''));
      反代IP = 读取路径.match(/ip=([^&]+)/)?.[1] || 反代IP;

      // Cloudflare Worker 内置 WebSocketPair：客户端端返回给访问者，服务端端留给 Worker 转发。
      ...
      return new Response(null, { status: 101, webSocket: 客户端 });
        ...
        // 定义节点信息显示路径；访问 https://部署域名/sub 可看到默认节点模板。
        const 节点路径 = '/sub';
...
async function 启动传输管道(WS接口, TCP接口) {
  // 这些变量在首包解析后会被后续的双向转发管道复用。
  // 传输队列用于串行化写入，避免多个 WebSocket message 同时写 TCP 导致顺序错乱。
  let 识别地址类型, 访问地址, 地址长度, 首包数据 = false, 首包处理完成 = null, 传输数据, 读取数据, 传输队列 = Promise.resolve();
  try {
    WS接口.addEventListener('message', async event => {
      if (!首包数据) {
        // 第一包包含 VLESS 请求头，需要先解析 UUID、目标地址、端口和初始数据。
        首包数据 = true;
        首包处理完成 = 解析首包数据(event.data);
        传输队列 = 传输队列.then(() => 首包处理完成).catch(e => { throw (e) });
      } else {
        // 首包解析完成后，其余 WebSocket 数据直接写入 TCP 连接。
        await 首包处理完成;
    ...
    async function 解析首包数据(首包数据) {
      ...
      // VLESS UUID 在首包第 1-16 字节，转换成标准 UUID 字符串后与配置比对。
      ...
      // VLESS 地址部分支持 IPv4、域名和 IPv6；不同类型的长度计算不同。
      ...
      // VLESS 响应头：协议版本 + 成功状态，随后启动 TCP -> WebSocket 回传。
      WS接口.send(new Uint8Array([协议头, 0]));
      启动回传管道();
    }
    async function 启动回传管道() {
      // 持续读取 TCP 返回数据并发送给 WebSocket 客户端，直到 TCP 流结束。
      while (true) {
        await 传输队列;
    ...
    // 任意阶段出错都关闭两侧连接，防止 Worker 里残留未释放的连接。
