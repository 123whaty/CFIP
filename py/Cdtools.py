...
# cdtools 按地区提供优选 IP 页面；这里聚合多个地区页面后统一排序输出。
# 如果某个地区页面临时不可用，main 会跳过该地区，尽量保留其他地区的结果。
REGION_URLS = [
...
def fetch_html(url: str, timeout_seconds: int = 20) -> str:
    ...
    trust_env=False 用来忽略运行环境里的代理变量，避免 GitHub Actions 或本地代理
    干扰抓取。raise_for_status 会把 4xx/5xx 转成异常，让 main 能清晰记录失败来源。
    """
...
# IP 可带端口，但输出只保留 IPv4 主体。
IP_REGEX = re.compile(r"\b((?:\d{1,3}\.){3}\d{1,3})(?::\d+)?\b")

# 支持示例：12.3 MB/s、850KB/s、1.2Gbps、500 Mb/s、900 kbit/s。
...
def normalize_speed_to_bytes_per_second(value_str: str, unit_str: str) -> float:
    """把各种速度单位统一成 bytes/s，便于不同页面结果比较排序。"""
...
def extract_ip_and_speed_from_element(el) -> List[Tuple[str, str, float]]:
    ...
    cdtools 页面结构变化时，表格解析可能失效；这个函数作为兜底解析卡片、
    列表、div 等块级文本，尽可能减少上游页面改版造成的空输出。
    """
...
def parse_ips_and_speeds(html: str) -> List[Tuple[str, str, float]]:
    ...
    优先读取结构化表格；表格列识别失败时，再尝试卡片/列表元素；最后用整页文本
    近邻匹配兜底。返回值已经按 IP 去重，并保留同一 IP 的最高速度。
    """
...
def main() -> int:
    ...
    workflow 会把 stdout 写入 Cdtools.txt；如果所有页面都失败，返回非 0，
    workflow 会保留旧文件，避免把可用列表清空。
    """
    ...
    for ip, speed_str, _ in pairs:
        format_output(ip, speed_str)
