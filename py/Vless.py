...
# 手工维护的候选域名池。
# 与 Domain.py 使用同一类测速方式，但最终输出完整 VLESS 节点链接。
RAW_ITEMS = [
...
async def measure_connect_latency_ms(domain: str, port: int = 443, timeout: float =1, attempts: int = 2) -> Optional[float]:
    ...
    这里只建立 TCP 连接，不做 TLS 握手和内容下载；延迟用于节点备注排序展示。
    多次尝试取最低值，失败则返回 None。
    """
...
async def main() -> None:
    ...
    节点模板里的 UUID、SNI、WS Host 和 Path 是项目默认公益配置；如需自用，
    可在这里替换成自己的部署信息。
    """

    domains = normalize_domains(RAW_ITEMS)
    results = await gather_latencies(domains)
    for domain, ms in select_top(results, 20):
